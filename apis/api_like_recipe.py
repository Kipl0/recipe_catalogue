...
def _():
    try:
        ...
        recipes_liked_by_user_record = db.execute(
            """SELECT * FROM recipes_liked_by_users
            WHERE recipes_liked_by_users_user_fk = ?
            AND recipes_liked_by_users_recipe_fk = ?""",
            (user_cookie["user_id"], recipe_id),
        ).fetchone()  # noqa

        recipe_liked = db.execute(
        ...
        if recipes_liked_by_user_record is None:
            db.execute(
                "INSERT INTO recipes_liked_by_users VALUES(?,?)",
                (user_cookie["user_id"], recipe_id),
            )  # noqa
            db.commit()
            ...
            db.execute(
                """UPDATE recipes
                SET recipe_total_likes = recipe_total_likes + 1
                WHERE recipe_id = ?""",
                (recipe_id,),
            )  # noqa
            db.commit()
        ...
        db.execute(
            """DELETE FROM recipes_liked_by_users
            WHERE recipes_liked_by_users_recipe_fk = ?""",
            (recipe_id,),
        )  # noqa
        db.commit()
        db.execute(
            """UPDATE recipes SET recipe_total_likes = recipe_total_likes - 1
            WHERE recipe_id = ?""",
            (recipe_id,),
        )  # noqa
        db.commit()
