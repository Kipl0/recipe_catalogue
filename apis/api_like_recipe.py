...
def _():
    try:
        ...
        if recipes_liked_by_user_record == None :
            db.execute("INSERT INTO recipes_liked_by_users VALUES(?,?)",(user_cookie["user_id"], recipe_id))
            db.commit()


            db.execute("UPDATE recipes SET recipe_total_likes = recipe_total_likes + 1 WHERE recipe_id = ?",(recipe_id,))
            db.commit()

            return {"info" : "ok", "recipe_id" : recipe_id, "recipe_total_likes" : int(recipe_total_likes) + 1}


        db.execute("DELETE FROM recipes_liked_by_users WHERE recipes_liked_by_users_recipe_fk = ?",(recipe_id,))
        db.commit()
        db.execute("UPDATE recipes SET recipe_total_likes = recipe_total_likes - 1 WHERE recipe_id = ?",(recipe_id,))
        db.commit()
