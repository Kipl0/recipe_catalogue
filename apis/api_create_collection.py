...
def _():
    try:
        ...
        #Upload thumbnail
        uploaded_thumbnail = request.files.get("collection_thumbnail_input") #files i formen
        if uploaded_thumbnail != None :
            ...
            if ext == "" : 
                ...
                uploaded_thumbnail.save(f"{rootdir}/images/collection_thumbnails/{final_thumbnail}")
        else :
            final_thumbnail = "default_collection.jpg"
        ...
        total_rows_inserted = db.execute(f"INSERT INTO collections VALUES({values})", collection).rowcount
        db.commit()
